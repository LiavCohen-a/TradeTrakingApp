...
router.route("/").post(async function (req, resp) {
  ...
  if (isValidPosition) {
    newPositionData.liquidityPrice = positionService.LiquidityPrice(
      newPositionData.leverage,
      newPositionData.entryPrice,
      newPositionData.type
    );
    newPositionData.size = positionService.sizeUSD(
      newPositionData.margin,
      newPositionData.leverage
    );
    ...
    if (newPositionData.type == "Long") {
      return resp.json(
        "Stop loss in long position must be less then entry price"
      );
    } else {
      return resp.json(
        "Stop loss in short position must be more then entry price"
      );
    }
  }
});
...
router.route("/:id").put(async (req, resp) => {
  ...
  if (updatedPositionData.positionClosed) {
    ...
    updatedPositionData.liquidityPrice = positionService.LiquidityPrice(
      updatedPositionData.leverage,
      updatedPositionData.entryPrice,
      updatedPositionData.type
    );
    updatedPositionData.size = positionService.sizeUSD(
      updatedPositionData.margin,
      updatedPositionData.leverage
    );
  }
  let dataResponse = await positionBL.UpdatePosition(
    positionID,
    updatedPositionData
  );
  return resp.json(dataResponse);
